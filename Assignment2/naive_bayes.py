#17EC30026 #Pranit Chawla #Assignment-2 #use python 17EC30026_2.py to run the code 
...
def load_data (file): #function to load the data as a list
	list_data=[]
...
def find_probs (train_data):  #finding probabilities for each attribute using laplaction smoothing
	...
	for i in range (1,7):
		for j in list_of_vals:
			...
			# print (i,j)
			p_happy_attributes[i][int(j)]=(float(len_ph)+1)/(len_happy+c) # using laplacian smoothig
...
accuracy=find_accuracy(test_data,p_happy_attributes,p_unhappy_attributes,p_happy,p_unhappy,c)
